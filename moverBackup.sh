...
echo "Starting backup your fucking site" \
    ...
    && docker exec -i mysql mysqldump -u root -proot wordpress > /home/$USER/projectku/backupsql/wordpress_"`date +"%d-%m-%Y"`".sql \
    && echo "Done" \
    && docker exec -i mysql mysqldump -u root -proot forum > /home/$USER/projectku/backupsql/forum_"`date +"%d-%m-%Y"`".sql \
